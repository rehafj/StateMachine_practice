...
///***********************************************  class members  **********************************///
	...
	Grid mygrid ; 
	public Transform player, Findingbot, t1, t2;
	public 	GameObject [] Redpointers;
	public 	GameObject []bluePointers;
	...
	public EnemyAI enemyState;

	public bool reachedEndRed;
...
///***********************************************  class methods   **********************************///
	...
	public void Update(){
		...
		Debug.Log(" reached next point if false draw path"+ reachedEndRed);
		timeLeft -= Time.deltaTime;

		if(enemyState.mystate == EnemyAI.States.chase){
		if(timeLeft<0){//if target found run pathfidning
			Debug.Log("Setting target to "+ newTarger);
			Debug.Log("finding the player");
			AstarPathFinding(Findingbot.position, player.position);
			timeLeft=1;
		}}
		if(enemyState.mystate == EnemyAI.States.patrol && !reachedEndRed){
		...
		if(timeLeft<0 && enemyState.mystate == EnemyAI.States.chase ){
			mygrid.StopCoroutine("MoveObjAlongPath");
			mygrid.path.Clear();
			mygrid.botPath.Clear();
			reachedEndRed = false;
		...
		if(!reachedEndRed){
		if( enemyState.mystate == EnemyAI.States.patrol ){
			chooseAmongActiveRed();
	...
	public Transform setTarget(){
		...
		else if ( enemyState.mystate == EnemyAI.States.patrol){
			Debug.Log("setting target to " + chooseAmongActiveRed());
			return chooseAmongActiveRed();
	...
	public Transform chooseAmongActiveRed(){
	if(Redpointers!=null){

		for( int i = 0; i < Redpointers.Length; i++){
			int chosen = Random.Range(0, Redpointers.Length-1);

			if(Redpointers[chosen].activeInHierarchy){
				//AstarPathFinding(Findingbot.position, Redpointers[i].transform.position);
					reachedEndRed = true;
					return Redpointers[chosen].transform;
	...
	public void chooseAmongActiveRedd(){
	if(Redpointers!=null){

		for( int i = 0; i < Redpointers.Length; i++){
			int chosen = Random.Range(0, Redpointers.Length-1);

			if(Redpointers[chosen].activeSelf){
				AstarPathFinding(Findingbot.position, Redpointers[i].transform.position);
					reachedEndRed = true;
					break;
	...
	public void SetGoals(){
		...
		Node PLayerNode = mygrid.GetNodeLocation(player.position);
	
		Node target1 = mygrid.GetNodeLocation(t1.position);											//Debug.Log("start node "+ start_pos.ToString());
		Node target2 = mygrid.GetNodeLocation(t2.position);	
		Node target3 = mygrid.GetNodeLocation(t2.position);	



		//cacuate the h cost and determaine target before path planning
	//	getPortHcost(startNode, goalNode, target1, target2);	
